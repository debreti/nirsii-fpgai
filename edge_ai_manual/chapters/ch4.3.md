...
Не каждый трансформер предназначен для расчёта **LLM**, но любая современная  современная языковая модель построена именно на этой архитектуре. Рассмотрим в начале те работы, результатами которых можно воспользоваться для анализа и обработки изображений и временных рядов.
...
Фреймворк ориентирован на компактную **encoder-only** архитектуру **Tiny Transformer** и поддерживает три ключевые задачи анализа временных рядов: прогнозирование (one-step ahead forecasting), классификацию и пороговое обнаружение аномалий.
...
Полный синтез проекта от запуска до битстрима на оборудовании авторов статьи занимает порядка **12 часов**. 
