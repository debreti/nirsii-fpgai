...
<a name="Compatibility_done"></a>
<figure>
  <img src="../../Compatibility_done.drawio.png" width="2000" alt="Классификация инструментов"/>
  <figcaption> Рисунок 3.1 – Классификация инструментов имплементации Edge AI на [FPGA](glossary.md#fpga)</figcaption>
</figure>
...
<a name="image_3_2"></a>
<figure>
  <img src="../assets/image_3_2.png" width="700" alt="Процесс глубокого обучения"/>
...
* Поддержка различных стратегий [квантизации](glossary.md#quantization) (PTQ, [QAT](glossary.md#qat)) и оптимизированных примитивов для целевых платформ;
* Генерация промежуточных представлений (ONNX, Relay) и целевых бинарных файлов/библиотек для компиляторов аппаратных ускорителей;
...
После оптимизации модели следует этап преобразования её в аппаратное представление, адаптируемое для синтеза на FPGA. Фреймворки адаптации решают эту задачу: они берут оптимизированную нейросетевую топологию и параметры (квантованные, фиксированной точности) и генерируют вход для [HLS](glossary.md#hls)-компилятора или прямо [RTL](glossary.md#rtl)-описание. Это звено критично, поскольку обеспечивает:

* Автоматическое или полуавтоматическое преобразование графа нейросети в [HLS](glossary.md#hls) С/С++ или в составные [IP](glossary.md#ip)-блоки;
* Интеграцию встроенных оптимизаций (конвейеризацию, развёртывание циклов, распределение памяти) для достижения целевых показателей по пропускной способности и задержке;
* Связь между высокоуровневым алгоритмом и низкоуровневыми деталями архитектуры FPGA (LUT, FF, BRAM, DSP);
* Упрощение процесса проектирования за счёт автоматизации рутинных операций преобразования.

Выбор инструмента адаптации зависит от целевой платформы (Xilinx/AMD, Intel/Altera, Lattice), уровня оптимизации и требуемого контроля над генерируемым кодом. Ниже описаны ключевые инструменты:
...
## **3.5 Инструменты размещения на FPGA** <a name="ch3.5"></a>

Основной этап физического проектирования FPGA — синтез, размещение и маршрутизация (Place & Route, P&R). На входе — [RTL](glossary.md#rtl)-описание ([VHDL](glossary.md#vhdl), [Verilog](glossary.md#verilog)), сгенерированное [HLS](glossary.md#hls)-компилятором или написанное вручную. На выходе — bitstream — бинарный файл, программирующий конкретную [FPGA](glossary.md#fpga). На этом этапе инструменты:

* Синтезируют [RTL](glossary.md#rtl) в логику управления, пригодную для целевого семейства FPGA;
* Применяют оптимизации синтеза (логическое развёртывание, факторизацию, декомпозицию);
* Размещают логику на доступные ресурсы платы (LUT, FF, BRAM, DSP, IO);
* Выполняют маршрутизацию соединений между размещёнными элементами;
* Проводят анализ временных ограничений ([timing analysis](glossary.md#timing)) и оценку мощности;
* Генерируют битстрим и отчёты по использованию ресурсов и производительности.

- [3.5.1 Vivado: инструмент размещения для микросхем Xilinx/AMD](ch3.5.1.md#ch3.5.1)
- [3.5.2 Quartus: инструмент размещения для микросхем Intel/Altera](ch3.5.2.md#ch3.5.2)

## **3.6 Многофункциональные инструменты и экосистемы реализации инференса нейронных сетей на FPGA** <a name="ch3.6"></a>
Одной из особенностей инструментов для разработки Edge AI решений является поддержка функционала разных этапов, например глубокого обучения и оптимизации (PyTorch) или наличие тесной взаимосвязи между инструментами от одного разработчика (OpenVINO, FPGA AI Suite, Quartus Prime).

В рамках данного пункта будут рассмотрены те инструменты, которые нельзя однозначно отнести к одному из предыдущих пунктов. 

* [3.6.1 OneWare: экосистема для ускоренной разработки Edge AI на FPGA](ch3.6.1.md#ch3.6.1)
* [3.6.2 Lattice: Sensai → Propel → Diamond/Radiant — экосистема для компактных ПЛИС](ch3.6.2.md#ch3.6.2)
