...
**OneWare** ориентируется на задачи **Vision AI**: *классификацию* изображений, *обнаружение объектов* на изображении, *сегментацию*, анализ видеопотока и другие приложения компьютерного зрения. При работе с  аудио-данными или данными временных рядов, их предварительно требуется преобразовать в спектрограммы, для чего представлен инструмент **Spectrogram Generator**. Причём пользователью представляется возможность икусственно увеличить размер набора данных выборки путём применения случайных преобразований к изображениям. Такие преобразования называются **Augmentations** и по сути являются методами технического зрения. Таким образом можно целеноправленно имитировать возможные изменения конкретных условий, которые могут возникнуть при эксплуатации, например изменение углов поворота объектов или условию освещения. 

При этом **OneWare** не привязан к конкретному вендору ПЛИС: система поддерживает экспорт платформонезависимого описания на **VHDL**, а также в промежуточные форматы описания нейронных сетей, **ONNX**, **TensorFlow** и **TensorFlow Lite**, для интеграции с другими тулчейнами.
...
<figure>
  <img src="../assets/image_3_6_1_4.png" alt="Вариант использования One AI в локальном окружении" width="450">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_5.png" alt="Вариант использования One AI в локальном окружении" width="800">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_6.png" alt="Вариант использования One AI в локальном окружении" width="300">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_7.png" alt="Вариант использования One AI в локальном окружении" width="600">
</figure>


Требуется сделать следующие замечания:
- Значение весов предполагается хранить только во **встроенной памяти** (internal SRAM) без использования **DDR**. *При этом никто не запрещает хранить оригинал изображения в DDR и сохрянять туда результаты расчётов, но это выходит за рамки генерируемого кода*;
- Разрядность квантованая задаётся одинаковой для всех слоёв нейронной сети (в отличие, например, от Brevitas);
...
<figure>
  <img src="../assets/image_3_6_1_8.png" alt="Вариант использования One AI в локальном окружении" width="350">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_9.png" alt="Вариант использования One AI в локальном окружении" width="500">
</figure>

Все фильтры являются **синтезируемыми** и будут включены в состав генерируемого IP-ядра. Хотя нужно сделать оговорку, что изменение разрешения изображения рекомендуется не включать в IP-ядро. Это связано с тем, что передача данных на вход посуществляется попиксельно и, поэтому мы снизим пропускную способность пропорционально выбранному коэффициенту.

<a name="image_3_6_1_10"></a>
<figure>
  <img src="../assets/image_3_6_1_10.png" alt="Вариант использования One AI в локальном окружении" width="800">
</figure>

Перечень доступных фильтров представлен ниже. 
<div style="display: flex; gap: 15px; margin-bottom: 25px; align-items: flex-start;">
  <div style="text-align: justify;">
    Изображение можно **обрезать** до заданной области интереса, применить **фильтр нижних частот**, чтоб удалить перепады на краях или наоборот выделить их **фильтром верхних частот**. Изменить яркость, контраст, насыщенность и пр. в **Color Filter**. Наиболее наглядным будет изменение параметров для набора данных из демо-вариантов.
  </div>
    <a name="image_3_6_1_11"></a>
  <figure style="margin: 0; flex-shrink: 0;">
    <img src="../assets/image_3_6_1_11.png" alt="Вариант использования One AI в локальном окружении" width="150" />
  </figure>
</div>
...
<div style="display: flex; gap: 15px; margin-bottom: 25px; align-items: flex-start;">
  ...
  <figure style="margin: 0; flex-shrink: 0;">
    <img src="../assets/image_3_6_1_12.png" alt="Вариант использования One AI в локальном окружении" width="600" />
  </figure>
</div>


Аугментации бывают двух типов: **статические** применяются превыми и их порядок является неизменным, **динамические** указываются пользователем и могут быть применены многократно. В целом смысл аугментаций достаточно понятен из названий:

<a name="image_3_6_1_13"></a>
<figure>
  <img src="../assets/image_3_6_1_13.png" alt="Вариант использования One AI в локальном окружении" width="500">
</figure>

На практике для любой аугментации будет дано поле для настроек и пример результата преобразования изображения:

<a name="image_3_6_1_14"></a>
<figure>
  <img src="../assets/image_3_6_1_14.png" alt="Вариант использования One AI в локальном окружении" width="400">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_15.png" alt="Вариант использования One AI в локальном окружении" width="150">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_16.png" alt="Вариант использования One AI в локальном окружении" width="800">
</figure>

После задания всех настроек можно запустить этап обучения, на выходе котрого мы получим в консоли краткую сводку о результатах поиска архитектуры и свойствах IP-ядра:

<a name="image_3_6_1_17"></a>
<figure>
  <img src="../assets/image_3_6_1_17.png" alt="Вариант использования One AI в локальном окружении" width="300">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_18.png" alt="Вариант использования One AI в локальном окружении" width="400">
</figure>
...
- Quartus_IP - IP-ядро One Ware для Quartus (например, для подключения сверточной нейронной сети к центральному процессору). На входе имеет интерфейс Avalon-MM Master для чтения из памяти изобрежения заданного формата, на выходе интерфейс Avalon-MM Slave для выдачи результата. TCL скрипт создаётся для qsys 25.1
...
<figure>
  <img src="../assets/image_3_6_1_19.png" alt="Вариант использования One AI в локальном окружении" width="300">
</figure>


Основная концепция: Потоковая передача

Все модули OneAI используют единый интерфейс потоковой передачи. Он используется как для передачи в сверточную нейронную сеть (CNN), так и между внутренними блоками. Определение типа данных для интерфейса CNN_Stream_T осуществляется файле ONEAI_Confic_Package.vhd. Интерфейс содержит следующие сигналы:
...
С этим интерфейсом связан интерфейс данных CNN_Values_T, представленный n-мерным сигналом, зависящим от количества цветовых каналов изображения. Каждый канал представляет собой 7-битный беззнаковый сигнал, кодирующий соответствующее значение пикселя в диапазоне 0...127.

В случае наличия нескольких входных Data-сигналов (например, при анализе нескольких изображений) все Data-сигналы должны соответствовать одному и тому же входному потоку, то есть они должны использовать общие сигналы тактирования, сигналы "валидности", столбцов, строк и фильтра.

<a name="image_3_6_1_20"></a>
<figure>
  <img src="../assets/image_3_6_1_20.png" alt="Вариант использования One AI в локальном окружении" width="350">
</figure>
...
<figure>
  <img src="../assets/image_3_6_1_21.png" alt="Вариант использования One AI в локальном окружении" width="450">
</figure>

<a name="image_3_6_1_22"></a>
<figure>
  <img src="../assets/image_3_6_1_22.png" alt="Вариант использования One AI в локальном окружении" width="1000">
</figure>


#### Вывод и размышления об ограничениях
Программа пока находится в стадии активной разработки, поэтому некоторые версии являются нерабочими. В частности версия 1.0.15 в какой-то момент потеряла работоспособнсоть, что потребовало переустановки. Функционал постепенно пополняется и видоизменяется.

Инструмент предназначен для преимущественно задач Vision AI, результатом синтеза всегда является свёрточная нейронная сеть, CNN. Для задач анализа **чего?** может быть избыточен, а для задач анализа временных рядов недостаточен всвязи с отсутствием поддержки обратной связи (**RNN**, **LTSM**). 

Код является платформонезависимым, но вопрос о полноте использования возможности DSP-блоков в разных семействах  и ПЛИС от разных вендоров остаётся открытым. Разработчики в большей степени ориентируются на взаимодействие с Altera, поэтому, например, использование AI Engines в AMD Versal остаётся "серой областью" (для авторов мануала тоже, поскольку не пробовали)