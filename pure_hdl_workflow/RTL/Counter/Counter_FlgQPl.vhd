library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;
use ieee.std_logic_misc.all;
library work;
use work.NPU_Package.all;

entity Counter_FlgQPl is
    generic
    (
        CNT_MAX : natural := 1
    );
    port
    (
        clk      : in  std_logic                                        := '0';
        aclr     : in  std_logic                                        := '0';
        en       : in  std_logic                                        := '0';
        load     : in  std_logic                                        := '0';
        in_data  : in  std_logic_vector(Log2Ceil(CNT_MAX) - 1 downto 0) := (others => '0');
        min      : out std_logic                                        := '0';
        max      : out std_logic                                        := '0';
        out_data : out std_logic_vector(Log2Ceil(CNT_MAX) - 1 downto 0) := (others => '0')
    );
end entity;

architecture rtl of Counter_FlgQPl is
    /* ┏━━━━━━━━━━━━━━━┓ */
    -- ┃    SIGNALS    ┃ --
    /* ┗━━━━━━━━━━━━━━━┛ */
    signal cnt : natural range 0 to (CNT_MAX - 1) := 0;
begin

    min      <= '1' when cnt = 0 else '0';
    max      <= '1' when cnt = (CNT_MAX - 1) else '0';
    out_data <= std_logic_vector(to_unsigned(cnt, Log2Ceil(CNT_MAX)));

    Seq_Logic_Arst : process(clk, aclr)
    begin
        if(aclr) then
            cnt <= 0;
        elsif(rising_edge(clk)) then
            if(load or en) then
                if(load) then
                    cnt <= to_integer(unsigned(in_data));
                else
                    if(max) then
                        cnt <= 0;
                    else
                        cnt <= cnt + 1;
                    end if;
                end if;
            end if;
        end if;
    end process;

end architecture;